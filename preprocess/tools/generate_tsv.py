...
def load_image_ids():
    ...
    with open("../data/processed/video_img_split.json") as f:
        temp = json.load(f)
        for i in temp.keys():
            for j in temp[i]:
                filepath = "../data/processed/video_imgs/" + j
                split.append((filepath, j.split(".")[0]))
    return split
...
def parse_args():
    ...
    parser.add_argument('--def', dest='prototxt',
                        ...
                        default='./preprocess/resnet101.prototxt', type=str)
    parser.add_argument('--net', dest='caffemodel',
                        help='model to use',
                        default='./preprocess/resnet101_faster_rcnn_final_iter_320000.caffemodel', type=str)
    parser.add_argument('--out', dest='outfile',
                        help='output filepath',
                        default='../data/processed/video_qa.tsv', type=str)
    parser.add_argument('--cfg', dest='cfg_file',
                        help='optional config file', default='./preprocess/faster_rcnn_end2end_resnet.yml', type=str)
